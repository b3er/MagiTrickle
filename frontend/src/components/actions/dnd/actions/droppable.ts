...
import { dndState } from "../stores/dnd.svelte.ts";
import type { DragDropOptions, DragDropState } from "../types/index.ts";
...
export function droppable<T>(node: HTMLElement, options: DragDropOptions<T>) {
  ...
  function handleDragStartOnContainer(_event: Event) {
    if (options.disabled) return;
  ...
  function handlePointerOver(_event: PointerEvent) {
  ...
  function handlePointerOut(_event: PointerEvent) {
    if (options.disabled || !dndState.isDragging) return;
